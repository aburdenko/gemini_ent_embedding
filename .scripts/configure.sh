...
if [ ! -d ".venv/python3.12" ]; then
  ...
  # --- Ensure UI/Browser libraries are installed for headless tools ---
  echo "Installing system libraries for browser/UI tools..."
  sudo apt-get update && sudo apt-get install -y libxkbcommon0 libxkbcommon-x11-0 libgbm1 libasound2t64
...
# Checksum logic for requirements.txt
...
if [ -f "$REQ_FILE" ]; then
    ...
    if [ "$CURRENT_HASH" != "$STORED_HASH" ]; then
        echo "requirements.txt changed or not installed. Installing dependencies..."
        ...
        if ./.venv/python3.12/bin/pip install --quiet --no-cache-dir -r requirements.txt; then
          ...
          # Don't update hash on failure so it retries next time
        fi
    else
        echo "Dependencies are up to date (hash match)."
    fi
fi
...
if ./.venv/python3.12/bin/pip show playwright &> /dev/null; then
  # Only run install-deps if the marker file doesn't exist
  if [ ! -f "$PLAYWRIGHT_MARKER" ]; then
      echo "Ensuring Playwright system dependencies are installed..."
      if ./.venv/python3.12/bin/python -m playwright install-deps; then
        touch "$PLAYWRIGHT_MARKER"
        echo "Playwright system dependencies installed and marker created."
      else
        echo "WARNING: Failed to install Playwright system dependencies." >&2
      fi
  else
      echo "Playwright system dependencies are up to date (marker found)."
  fi
fi
